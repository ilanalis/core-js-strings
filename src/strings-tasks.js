...
function removeFirstOccurrences(str, value) {
  const startIndexValue = str.indexOf(value);
  if (startIndexValue < 0) return str;
  return (
    str.slice(0, startIndexValue) +
    str.slice(startIndexValue + value.length, str.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const startIndexValue = str.lastIndexOf(value);
  if (startIndexValue < 0) return str;
  return (
    str.slice(0, startIndexValue) +
    str.slice(startIndexValue + value.length, str.length)
...
function sumOfCodes(str) {
  ...
  const arrayOfCharacterCodes = str
    .split('')
    .map((character) => character.charCodeAt());
  const sumOfCharactersCodes = arrayOfCharacterCodes.reduce((prev, current) => {
    return prev + current;
  }, 0);
  return sumOfCharactersCodes;
}
...
function formatTime(minutes, seconds) {
  const getFormattedUnitOfTime = (unitOfTime) => {
    return String(unitOfTime).padStart(2, '0');
  };
  return `${getFormattedUnitOfTime(minutes)}:${getFormattedUnitOfTime(
    seconds
  )}`;
}
...
function orderAlphabetically(str) {
  const arrayOfString = str.split('');
  const sortedArray = arrayOfString.sort((a, b) => a.localeCompare(b));
...
function isPalindrome(str) {
  ...
  const arrayOfString = str.toLowerCase().replaceAll(regExp, '').split('');
  const reversedArray = [...arrayOfString].reverse();
  return arrayOfString.join('') === reversedArray.join('');
}
...
function findLongestWord(sentence) {
  ...
  const sortedArray = arrayOfString.sort((a, b) => {
...
function reverseWords(str) {
  const arrayOfString = str.split(' ');
  const reversedArray = arrayOfString.map((word) => {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!input.includes(str[i])) {
      ...
      const indexOfLetter = input.indexOf(str[i]);
      newStr += encoded[indexOfLetter];
    }
  }
  return newStr;
}
...
function getCardId(value) {
  ...
  const character = value.slice(0, value.length - 1);
  const symbol = value[value.length - 1];
  const index = arrayOfCharacter.indexOf(character) + objOfSymbols[symbol];
  return index;
}
